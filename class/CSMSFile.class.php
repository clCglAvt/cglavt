 ...
 *
 * MODIF CCA 5-3-2019 - Mise en place Sms
 *
...
class CSMSFile
{
	 ...
	 *	CSMSFile
	 *
	 *	@param	array	$to                 Recipients SMS
	 *	@param 	string	$from               Sender SMS
	 *	@param 	string	$msg                Message
	 *	@param 	array	$filename_list       List of files to attach (full path of filename on file system)
	 *	@param 	array	$mimetype_list       List of MIME type of attached files
	 *	@param 	array	$mimefilename_list   List of attached file name in message
	 *	@param 	int		$idbull				Identifiant du bulletin	
	 * $return int 0 ou <0=NonOK, 1= OK
	 
	 */
	function __construct($to,$from,$msge,$filepath,$mimetype,$filename, $idbull = '')
	{
		global $conf;

		
	dol_syslog('CCA -- __construct - to:'.$to);
	dol_syslog('CCA -- __construct - from:'.$from);
	dol_syslog('CCA --__construct - >msg:'.$msg);
	...
	dol_syslog('CCA -- sendfile - this->addr_to:'.$this->addr_to);
	dol_syslog('CCA -- sendfile - this->sendcontext:'.$this->sendcontext);
	dol_syslog('CCA -- sendfile - this->sendmode:'.$this->sendmode);
	dol_syslog('CCA -- sendfile - this->headers:'.$this->headers);
	dol_syslog('CCA -- sendfile - this->addr_from:'.$this->addr_from);
	dol_syslog('CCA -- sendfile - this->message:'.$this->message);
	dol_syslog('CCA -- sendfile - this->smtps:'.$this->smtps);
...
// Modif CCA  5-3-2019 - Mise en place Sms
	/*
	* Vérifie que le téléphone es un mobile 
	* Voir Protocole pour les téléphones étrangers - à définir
	*
	* 	@param $tel		numéro de téléphone à vérifier
	* 	@param $pays	code du pays
	*/
	function getValidTel($tel,  $pays = '') 
	{
