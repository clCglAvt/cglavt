 ...
 *
 * MODIF CCA - 31/12/14 Suivi Client 
 * Modif CCA sept 2017 -  Ajout déposit
 * MODIF MDU 19/05/21 - réintégration du menue des comptes bancaires supprimé en V12	
 * Modif CCA fev 2023 - Ajout Suivi Activite
 *
...
/**
 *  copy de 		htdocs/core/menus/standard/eldy.lib.php
// Modif CCA sept 2017 -  Ajout déposit
 *  \file		htdocs/core/menus/standard/eldy_deposit.lib.php
 *  \brief		Library for file eldy + déposit menus
// FIN Modif CCA sept 2017 -  Ajout déposit
 */
...
function print_eldy_deposit_menu($db, $atarget, $type_user, &$tabMenu, &$menu, $noout = 0, $mode = '')
// FIN Modif CCA sept 2017 -  Ajout déposit
{
...
//	$menuArbo = new Menubase($db, 'eldy');
	$menuArbo = new Menubase($db, 'eldy_deposit');
//	$newTabMenu = $menuArbo->menuTopCharger('', '', $type_user, 'eldy', $tabMenu);	// Return tabMenu with only top entries
	$newTabMenu = $menuArbo->menuTopCharger('', '', $type_user, 'eldy_deposit', $tabMenu);	// Return tabMenu with only top entries
// FIN Modif CCA sept 2017 -  Ajout déposit
...
//  function print_left_eldy_menu($db, $menu_array_before, $menu_array_after, &$tabMenu, &$menu, $noout = 0, $forcemainmenu = '', $forceleftmenu = '', $moredata = null, $type_user = 0)
	function print_left_eldy_deposit_menu($db, $menu_array_before, $menu_array_after, &$tabMenu, &$menu, $noout = 0, $forcemainmenu = '', $forceleftmenu = '', $moredata = null, $type_user = 0)
// FIN Modif CCA sept 2017 -  Ajout déposit
{
...
//MODIF CCA  - 31/12/14 Suivi Client 
				$newmenu->add("/custom/CahierSuivi/suivi_client/list_cahier.php?typeliste=generale", $langs->trans("Suivi Clientele"), 0, $user->rights->societe->lire, '', $mainmenu, 'thirdparties');
// Fin MODIF CCA				
...
// Modif CCA fev 2023 - Ajout Suivi Activite
					$newmenu->add("/custom/cglinscription/RapportProdSecteur.php?typeRapport=CA", $langs->trans("SuiviActivite"), 2, $user->rights->accounting->comptarapport->lire);
// Fin Modif CCA fev 2023
...
// Modif CCA sept 2017 -  Ajout déposit
		...
		$newmenu = $menuArbo->menuLeftCharger($newmenu,$mainmenu,$leftmenu,(empty($user->societe_id)?0:1),'eldy_deposit',$tabMenu);
// FIN Modif CCA sept 2017 -  Ajout déposit
		//var_dump($newmenu->liste);    //
// MODIF MDU 19/05/21 - réintégration du menue des comptes bancaires supprimé en V12		
...
// Fin MODIF MDU 19/05/21 - réintégration du menue des comptes bancaires supprimé en V12	
