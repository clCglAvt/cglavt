 ...
 *
 * MODIF CCA 1/12/2019 - Récupère le nom du propriétaire de la carte
 * MODIF CCA 04/01/2020 - Vérification validité demande Stripe
 * Modif CCA 23/5/2020 - Attente intégration dans Dolibarr de https://www.dolibarr.fr/forum/t/stripe-fonctionnement-module/30532/7 (sylvain Legrand sept 19)
 * MODIF CCA 21/7/2020  - Applique vérification faite 
 *
...
//MODIF CCA 1/12/2019 - Récupère le nom du propriétaire de la carte
$cardholder_name = GETPOST("cardholder-name", 'alpha');
//Fin MODIF CCA 1/12/2019
...
//MODIF CCA 21/7/2020 CCA Applique vérification faite 
//	$error = 0;
	$error = $IspaiementFait;
//Fin CCA MODIF  21/7/2020 CCA
	$errormessage = '';

	// When using the old Charge API architecture
//MODIF CCA 21/7/2020 CCA Applique vérification faite 
    if (!$error and empty($conf->global->STRIPE_USE_INTENT_WITH_AUTOMATIC_CONFIRMATION)) {
//    if (empty($conf->global->STRIPE_USE_INTENT_WITH_AUTOMATIC_CONFIRMATION)) {
//Fin MODIF CCA 21/7/2020 CCA
		try {
			$metadata = array(
...
//MODIF CCA 1/12/2019 -Récupère le nom du propriétaire de la carte
	$_SESSION['cardholder-name'] = $cardholder_name ;
//Fin CCA MODIF  1/12/2019
 ...
 * View
 */
// MODIF CCA 04/01/2020 - Vérification validité demande Stripe car en mode 'free', Dolibarr ne fait pas de vérification		
if ( empty($source)) {	
	require_once DOL_DOCUMENT_ROOT.'/custom/cglinscription/class/bulletin.class.php';
		...
		// Vérification date de paiement de bulldet5 ==> demande déjà payée 
		$IspaiementFait = false;
		$fbulldet5 = new Bulletin ($db);
		$ret = $fbulldet5->fetchDemandesStripe ('', $id_acompte) ;
		if ($ret <=0) {
			setEventMessages($langs->trans("ErrorStrpeAnnule"), null, 'errors');
			$IspaiementFait = true;
		}
		// Vérification présence de AC??? demande non supprimée
		if ($fbulldet5->action == 'S' or  $fbulldet5->action =='X') {
			setEventMessages($langs->trans("ErrorStrpeAnnule"), null, 'errors');
			$IspaiementFait = true;
		}
		if (! empty($fbulldet5->datepaiement)) {
			setEventMessages($langs->trans("ErrorStrpeDejaPaye"), null, 'errors');
			$IspaiementFait = true;
		}
		unset($fbulldet5);
}
// Fin MODIF CCA 4/1/2020 
$form = new Form($db);
...
// Fin MODIF CCA 23/5/2020  Attente intégration dans Dolibarr
...
// MODIF CCA 04/01/2020 - Vérification validité demande Stripe
			//if ((empty($paymentmethod) || $paymentmethod == 'stripe') && !empty($conf->stripe->enabled)){
			if (!($IspaiementFait) && (empty($paymentmethod) || $paymentmethod == 'stripe') && ! empty($conf->stripe->enabled))
// Fin CCA MODIF 				
			{
