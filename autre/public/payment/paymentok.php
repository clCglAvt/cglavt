 ...
 *
 * Modif CCA 15 juil 2020 pour éviter le raffraichissement et la création en double des éléments dans Dolibarr
 * Modif CCA 26/01/2021 Mettre la date de l'utilisateur et non du serveur
 *
...
// Modif CCA 15 juil 2020 pour éviter le raffraichissement et la création en double des éléments dans Dolibarr
if ( empty($_SESSION["FinalPaymentAmt"]) or empty($_SESSION["TRANSACTIONID"])) exit;
// Fin Modif CCA 15 juil 2020 
...
//Modif CCA 26/01/2021 Mettre la date de l'utilisateur et non du serveur
										...
										$sql .= " VALUES (".((int) $object->fk_soc).", '', '".$db->escape($stripecu)."', 'stripe', " . $db->escape($stripearrayofkeysbyenv[$servicestatus]['publishable_key'])."', ".((int) $servicestatus) . ", "((int) $conf->entity) . ", '".$db->idate(dol_now('tzuser'))."', 0)";
// Fin Modif CCA 26/01/2021								
										$resql = $db->query($sql);
