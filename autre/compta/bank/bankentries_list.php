 ...
 *
 * MODIF CCA 1/12/2018 - AJOUT COLONNE BORDEREAU DE DEPOT	- Vérification Codage du relevé
 * MODIF CCA 1/02/19 - Vérification Format num relevé
 * MODIF CCA 21/03/22 - Parametre de print_barre_liste pour afficher un changement de page correct
 *
...
// MODIF CCA 1/12/2018 - AJOUT COLONNE BORDEREAU DE DEPOT	
...
// MODIF CCA 1/12/2018 - AJOUT COLONNE BORDEREAU DE DEPOT	
$search_deposit=GETPOST("search_deposit",'int');
// FIN MODIF CCA 1/12/2018 	
...
// MODIF CCA 1/12/2018 - AJOUT COLONNE BORDEREAU DE DEPOT	
	'bc.ref'=>array('label'=>$langs->trans("BordereauDepot"), 'checked'=>1, 'position'=>800),
// MODIF CCA 
...
// MODIF CCA 1/12/2018 - AJOUT COLONNE BORDEREAU DE DEPOT	
if (!empty($search_deposit)) 
	$param.='&search_deposit='.urlencode($search_deposit);
// FIN MODIF CCA 1/12/2018 	
...
// MODIF CCA 1/12/2018 - AJOUT COLONNE BORDEREAU DE DEPOT
$sql.= ", b.fk_bordereau, bc.amount as TotDeposit,  bc.nbcheque, bc.date_bordereau , bc.ref as RefBordereau ";
// FIN MODIF CCA 1/12/2018 
...
// MODIF CCA 1/12/2018 - AJOUT COLONNE BORDEREAU DE DEPOT	
if ($search_deposit) 
	$sql.= ' AND b.fk_bordereau = '. $search_deposit;
// FIN MODIF CCA 1/12/2018 
if ($search_conciliated != '' && $search_conciliated != '-1') {
...
// MODIF CCA 1/02/19 - Vérification Format num relevé
		// Vérification de la forme du numéro de relevé saisi
		// La vérification concerne la longueur de la saisie (6 pour le format AAAAMM, 8 pour le format AAAAMMDD)
		// La vérification concerne le signalement que l'on n'est pas sur un relevé concernant le mois courant ou le mois précédent

print '<script type="text/javascript">
					function setEventMessage(text) {
						var block = false;
						var texte = new String (text)
						if (block) {
							$.dolEventValid("",texte);
						} else {
		...
		print ' <script type="text/javascript">
				$("input#num_releve").change(function() {
				var anneemois;
				var anneemoisprec;
				var anneemoissaisi;
				anneemoissaisi = jQuery(this).val() ;
				anneemois = Math.trunc('.dol_print_date(dol_now(),"dayxcard").'/100);	
				anneemoisprec = anneemois -1
				if (anneemoissaisi.length != 8 && anneemoissaisi.length  != 6)
					setEventMessage("Erreur de longueur");
				else {
					var fl1;
					var fl2;
					fl1 = (anneemoissaisi.search(anneemoisprec) == -1);
					fl2 =  (anneemoissaisi.search(anneemois) == -1);
					if (!(anneemoissaisi.search(anneemoisprec) >=0 || anneemoissaisi.search(anneemois)  >=0 ))   setEventMessage("la valeur ne correspond pas au mois courant ou précédent  ");	
				}
				/* si longueur <> 8 ou 6 erreur
				-- si 6 permiers caractères <> annee-mois courant  ==>  Erreur
				-- si 6 permiers caractères <> annee.mois courant-1 ==> Erreur
				-- Afficher le résultat de la boite setEventMessages($prodcustprice->error,
				-- $prodcustprice->errors, "errors");
				--$("input[type=checkbox][name^=rowid]").prop("checked", $(this).is(":checked"));
				*/
			});
			</script>';
// FIN MODIF CCA 1/02/19 - Vérification Format num relevé
		print '</div>';
...
// MODIF CCA 1/12/2018 - AJOUT COLONNE BORDEREAU DE DEPOT	
	if (! empty($arrayfields['bc.ref']['checked']))
	{
		...
		print select_bordereau_non_rapproches($search_account, $search_deposit, 'search_deposit',1);
     	print '</td>';
	}	
// FIN MODIF CCA  1/12/2018
...
// MODIF CCA 1/12/2018 - AJOUT COLONNE BORDEREAU DE DEPOT	
		...
		if (! empty($arrayfields['bc.ref']['checked']))
		{
			...
			$Deposit->fetch($objp->fk_bordereau);
			print '<td class="nowrap" align="center">';
			print $Deposit->getNomUrl(1);
			print "</td>\n";
...
/**
 *	Chercher les bordereaux non rapprochés à sélectionner
 *
 *	@param		int			$idaccount			Identifiant Compte bancaire
 *	@param		int			$selected			Identifiant bordereau dépot sélectionné
 *	@param		string		$htmlname			Nom de variable de passage url
