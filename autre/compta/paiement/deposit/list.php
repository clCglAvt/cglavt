...
if ($resql)
{
	...
	print_barre_liste($langs->trans("MenuDeposits" ), $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, '', $num, $nbtotalofrecords, 'title_bank.png', '', $morehtml, '',$limit, 0, 0, 1);
		$moreforfilter='';
    ...
    if ($num > 0)
    {
    	while ($i < min($num,$limit))
    	{
			...
			if (empty($objp->TotalMtt)) $objp->TotalMtt= 0;
			if (empty($objp->amount)) $objp->amount= 0;
