	...
	**/
	...
	function create($user, $account_id, $mode_paiementid, $limit, $toRemise)
	{
		...
		if ( $resql )
		{
			...
			if ($this->id > 0 && $this->errno == 0)
			{
				...
				$sql.= " WHERE  (b.amount > 0 or (b.amount < 0 and fl_regroupneg = 1 ))";
	...
	function getNextNumRef($mode='next')
	{
		...
		if (! empty($conf->global->CHEQUERECEIPTS_ADDON))
		{
			...
			$obj = new $classname();
			$numref = "";
			$prefixe = $this->paiementcode.'-';
			$numref = $obj->getNextValue($mysoc,$this,$prefixe);
	...
	function rejectCheck($bank_id, $rejection_date)
	{
		...
		if ($resql)
		{
			...
			$rejectedPayment->num_payment = $payment->numero;
