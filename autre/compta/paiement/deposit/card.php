 ...
 * 
 * Modif CCA 1/6/2018 - fl_regroup et fl_regroupneg
 * Modif CCA prête pour la V7
 * Modif CCA 19/5/2021 - Montée de version PHP
 * Version CAV - 2.7.1 - automne 2022 - Racupération du fichier d'édition du bordereau
 * Version CAV - 2.8.2 - introduction de la table payment_various pour la liste des dépôts à faire
 * Modif CCA 21/03/23 - ajout tables payment_various et paiementfourn
 * Modif CCA - 2.8.4 - 25/7/23 -  Lien Paiement Fourn corrigé	- pas de lien de paimement pour les paiement divers	
...
// Fin Modif CCA 19/5/2021 - Montée de version PHP
...
if ($action == 'confirm_reject_check' && $confirm == 'yes' && $user->rights->banque->cheque)
{
	...
	$rejected_check = GETPOST('banqueid');
...
*/
	dol_fiche_head($head, $hselected, $langs->trans("TiDeposit"),0,'payment');
	/*
	...
	{
		$formquestion = array(
			array('type' => 'hidden','name' => 'banqueid','value' => GETPOST('lineid')),
...
if ($action != 'new')
{
	if ($object->statut == 1)
	{
		...
		$filedir= $dir .get_exdir($object->ref,0,1,0,$object,'cheque') ;
		$urlsource=$_SERVER["PHP_SELF"]."?id=".$object->id;
		// On utilise remisechèque qui est une classe déterminant le modèle de bordereau  vraiment similaire à déposit
...
function 	AfficheHautBordereauNouv($filteraccountid, $filterpaiementid, $filterdate)
{
	...
	print '<form class="nocellnopadd" action="'.$_SERVER["PHP_SELF"].'?accountid='.$filteraccountid."&paiementid=".$filterpaiementid.'" method="POST">';
...
*/
	print '<tr><td width="200">'.$langs->trans("DateReceived").'</td><td>';
	print $form->select_date($filterdate,'fd',0,0,1,'',1,1,1);
	print '</td></tr>';
    print '<tr><td>'.$langs->trans("banqueAccount").'</td><td>';
...
function AfficheBordereau($object, $action)
{
	...
	$form = new Form($db);
/* Modif CCA prête pour le V7
	$linkback='<a href="'.DOL_URL_ROOT.'/compta/paiement/cheque/list.php">'.$langs->trans("BackToList").'</a>';
*/
	$linkback='<a href="'.$_SERVER["PHP_SELF"].'?leftmenu=customers_bills_checks&action=new&accountid='.$filteraccountid."&paiementid=".$filterpaiementid.'>'.$langs->trans("BackToList").'</a>';
		$paymentstatic=new Paiement($db);
	$accountlinestatic=new AccountLine($db);
...
// Espcase spécifique V4
	...
	if (empty($conf->global->DEPOSIT_REFEXT)) 
			print info_admin('Pour gerer la reference bancaire, valoriser la variable DEPOSIT_REF
		
		EXT', 1);
	print '</td>';
	if ($object->statut  == 1 and $action != 'editref') print '<td align="right"><a href="'.$_SERVER["PHP_SELF"].'?action=editref&amp;id='.$object->id.'">'.img_edit($langs->trans('SetRef'),1).'</a></td>';
	print '</tr></table>';
...
// fin espace spécifique V4
	...
	print '</td></tr>';	
	if (empty($object->TotalPaiement)) $object->TotalPaiement= 0;
...
function AfficheNonRemis($id, $filteraccountid, $filterpaiementid, $filterdate, $accounts, $user,  $titre)
{
	...
	// Paiement à déposer provenant des factures clients
//  Modif CCA - 2.8.4 - - Lien Paiement Fourn corrigé		
	$sql = "SELECT 'Fact' as Origine, ba.rowid as bid, b.datec as datec, b.dateo as date, b.rowid as transactionid, ";
// Fin Modif CCA - 2.8.4 - - Lien Paiement Fourn corrigé		
...
/* Modif CCA prête V7	
	$sql.= " WHERE ba.entity IN (".getEntity('banque_account').")";
*/
	$sql.= " WHERE ba.entity IN (".getEntity('banque_account', 1).")";
	$sql.= " AND b.fk_bordereau = 0";
	$sql.= " AND  ! isnull(p.fk_paiement)  ";
// Modif CCA 1/6/2018 - fl_regroup et fl_regroupneg
$sql.= " AND (b.amount > 0 or (b.amount < 0 and fl_regroupneg = 1 ))";
//	Fin Modif CCA 1/61/2018 - fl_regroup et fl_regroupneg
	$sql.= " and p.fk_paiement <> 2 ";
	$sql.= " and courant <> 2 ";
	$sql.= " AND b.fk_type <> 'VIR'";
	$sql.= "  AND not isnull(tp.id)";
	$sql.= "  AND NOT EXISTS (select (1) FROM ".MAIN_DB_PREFIX."bank_url as burl WHERE burl.url_id = b.rowid and type = 'banquetransfert') ";
// Modif CCA 21/03/23 - ajout tables payment_various et paiementfourn
	$sql.= "  AND YEAR(b.datec) > 2022 ";
// Fin Modif CCA  21/03/23 	
	...
	// Paiement à déposer provenant des acomptes fournisseurs
	$sql.= " UNION 	";
// Modif CCA - 2.8.4 - - Lien Paiement Fourn corrigé		
	$sql .= "SELECT 'Divers', ba.rowid as bid, b.datec as datec, b.dateo as date, b.rowid as transactionid, ";
// Fin Modif CCA - 2.8.4 - - Lien Paiement Fourn corrigé		
	$sql.= " b.amount, ba.label, b.emetteur, b.num_chq, b.banque,";
	$sql.= " p.rowid as paymentid, tp.code as paiementcode, tp.libelle as paiementlib, tp.id as mode_paiementid";
	$sql.= " FROM ".MAIN_DB_PREFIX."bank as b";
	$sql.= " LEFT JOIN ".MAIN_DB_PREFIX."payment_various as p ON p.fk_bank = b.rowid";
	$sql.= " LEFT JOIN ".MAIN_DB_PREFIX."bank_account as ba ON (b.fk_account = ba.rowid)";
	$sql.= " LEFT JOIN ".MAIN_DB_PREFIX."c_paiement as tp ON (p.fk_typepayment = tp.id)";
	$sql.= " WHERE ba.entity IN (".getEntity('banque_account', 1).")";
	$sql.= " AND b.fk_bordereau = 0";
	$sql.= " AND  ! isnull(p.fk_typepayment)  ";
	$sql.= " AND (b.amount > 0 or (b.amount < 0 and fl_regroupneg = 1 ))";
	$sql.= " and p.fk_typepayment <> 2 ";
	$sql.= " and courant <> 2 ";
	$sql.= " AND b.fk_type <> 'VIR'";
	$sql.= "  AND not isnull(tp.id)";
	$sql.= "  AND NOT EXISTS (select (1) FROM ".MAIN_DB_PREFIX."bank_url as burl WHERE burl.url_id = b.rowid and type = 'banquetransfert') ";
...
// Modif CCA 21/03/23 - ajout tables payment_various et paiementfourn
	...
	// Paiement à déposer provenant des paiements divers
	$sql.= " UNION 	";
// Modif CCA - 2.8.4 - - Lien Paiement Fourn corrigé		
	$sql .= "SELECT 'Fourn', ba.rowid as bid, b.datec as datec, b.dateo as date, b.rowid as transactionid, ";
// Fin Modif CCA - 2.8.4 - - Lien Paiement Fourn corrigé		
	$sql.= " b.amount, ba.label, b.emetteur, b.num_chq, b.banque,";
	$sql.= " p.rowid as paymentid, tp.code as paiementcode, tp.libelle as paiementlib, tp.id as mode_paiementid";
	$sql.= " FROM ".MAIN_DB_PREFIX."bank as b";
	$sql.= " LEFT JOIN ".MAIN_DB_PREFIX."paiementfourn as p ON p.fk_bank = b.rowid";
	$sql.= " LEFT JOIN ".MAIN_DB_PREFIX."bank_account as ba ON (b.fk_account = ba.rowid)";
	$sql.= " LEFT JOIN ".MAIN_DB_PREFIX."c_paiement as tp ON (p.fk_paiement = tp.id)";
	$sql.= " WHERE ba.entity IN (".getEntity('banque_account', 1).")";
	$sql.= " AND b.fk_bordereau = 0";
...
// Fin Modif CCA  21/03/23 	
	...
	else $sql.= $db->order("bid, mode_paiementid, date, transactionid ","ASC");
	$resql = $db->query($sql);
...
// Modif CCA - 2.8.4 - - Lien Paiement Fourn corrigé		
			$lines[$key][$i]["origine"] = $obj->Origine;
// Fin Modif CCA - 2.8.4 - - Lien Paiement Fourn corrigé		
			$i++;
		}
...
*/
// Espace spécifique V4 
			print '<table class="noborder" width="100%">';
// Fin espace spécifique
			print '<tr class="liste_titre">';
			//	print '<td style="min-width: 120px">'.$langs->trans("DateChequeReceived").'</td>'."\n";
			print_liste_field_titre("DateReceived",$_SERVER["PHP_SELF"],"date, transactionid", "",$param,'align="center"',$sortfield,$sortorder);
			print "</td>\n";
			...
			//print '<td style="min-width: 200px">'.$langs->trans("banque")."</td>\n";
			print_liste_field_titre("banque",$_SERVER["PHP_SELF"],"b.banque", "",$param,"",$sortfield,$sortorder);
			...
			print_liste_field_titre("Payment",$_SERVER["PHP_SELF"],"paymentid", "",$param,'align="center"',$sortfield,$sortorder);
			//print '<td align="center" width="100px">'.$langs->trans("LineRecord")."</td>\n";
			print_liste_field_titre("LineRecord",$_SERVER["PHP_SELF"],"transactionid", "",$param,'align="center"',$sortfield,$sortorder);
...
elseif ( $value["origine"] == 'Fourn') {
	// Paiement fournisseur ou divers
	if ($value["paymentid"]) {
		print getNomUrl( $value["origine"], $value["paymentid"]);
	}
	else {
						print '&nbsp;';
	}
...
// Fin Modif CCA - 2.8.4 - - Lien Paiement Fourn corrigé		
					...
					print '</td>';
					// Link to banque transaction
					print '<td align="center">';
// Fin Modif CCA - 2.8.4 - - Lien Paiement Fourn corrigé	
					$accountlinestatic->id=$value["id"];
					if ($accountlinestatic->id)
// Fin Modif CCA - 2.8.4 - - Lien Paiement Fourn corrigé	
					{
 ...
 function AfficheRemis($object, $accounts )
{
	...
	$paymentstatic=new Paiement($db);
	// List of paiements
	$sql = "SELECT b.rowid as transactionid, b.amount, b.num_chq, b.emetteur,";
	...
	if (! $sortfield) $sortfield="date,transactionid";	
...
/*
* Prépare le lien vers le paiement fournisseur ou divers
*
*	@param	string	$origine	Fourn pour un paiement fournisseur
*	@param	int		$id			identifiant du paaiement
*
*	retour 	string	$result 		Code html pour 	lien

*/
function getNomUrl( $origine, $id)
{
		if ($origine == 'Fourn')		{
			$result = '<a href="'.DOL_URL_ROOT.'/fourn/paiement/card.php?id='.$id.'" >' ;
			$result .= '<img border = 0 title="Lien" src="'.DOL_URL_ROOT.'/theme/eldy/img/object_payment.png" ></img>';
			$result .= '&nbsp'.$id.'</a>';
		}	
		return $result;
