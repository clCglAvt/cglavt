 ...
 *
 * Modif CCA 10/1/2018 - ajout colones type,accountancy_code et  fl_regroup et fl_regroupneg  pour dictionnaire mode paiement
 * Modif CCA 14/2/2018 - ajout colone fl_type et fk_produit pour dictionnaire raison remise
 * Modif CCA 15/12/2016 - ajout type couleur
 * Modif CCA 10/12/2022 - ajout colonne  fk_cpt_bq  dans c_paiement
 * 
...
// Modif CCA 10/1/2018 - ajout colone fl_regroup et fl_regroupneg
...
// Modif CCA 10/12/2022 - ajout colonne  fk_cpt_bq  dans c_paiement
$tabsql[13] = "SELECT c.id    as rowid, c.code, c.libelle, c.type, c.active, c.accountancy_code, c.fl_regroup , c.fl_regroupneg, c.fk_cpt_bq  FROM ".MAIN_DB_PREFIX."c_paiement AS c";
// fin  Modif CCA 10/1/2018 et 10/12/2022 
...
$tabfield[13] = "code,libelle,type,accountancy_code,fl_regroup,fl_regroupneg,fk_cpt_bq";
// Fin Modif CCA 10/1/2018  et 10/12/2022
...
$tabfieldvalue[13]= "code,libelle,type,accountancy_code,fl_regroup,fl_regroupneg,fk_cpt_bq";
// Fin Modif CCA 10/1/2018 et  10/12/2022
...
$tabfieldinsert[13]= "code,libelle,type,accountancy_code,fl_regroup,fl_regroupneg,fk_cpt_bq";
// Modif CCA 10/1/2018  et 10/12/2022
...
$tabhelp[13] = array('code'=>$langs->trans("EnterAnyCode"), 'fl_regroup'=>$langs->trans("FlRegroup"), 'fl_regroupneg'=>$langs->trans("FlRegroupNeg"), 'fk_cpt_bq'=>$langs->trans("Fk_cpt_bq"));
// Modif CCA 10/1/2018 et 10/12/2022
...
//Modif CCA 10/1/2018 - ajout colone fl_regroup et fl_regroupneg 
// Modif CCA 10/12/2022 - ajout colonne  fk_cpt_bq  dans c_paiement
        	&& (! in_array($listfield[$f], array('fl_regroup','fl_regroupneg', 'fk_cpt_bq)'))) 
// Fin 	Modif CCA 10/1/2018 et 10/12/202
...
// Modif CCA 10/1/2018 - Ajout colonne fl_regroup et fl_regroupneg 
	if (empty($_POST["fl_regroup"] ))  
		$_POST["fl_regroup"]=0;
	if (empty($_POST["fl_regroupneg"] ))  
		$_POST["fl_regroupneg"]=0;
// Fin Modif CCA 
...
// Modif CCA 10/1/2018 - ajout colone fl_regroup et fl_regroupneg pour dicitonnaire mode paiement
            if ($fieldlist[$field]=='fl_regroup'){ 
				$valuetoshow=$langs->trans("FlRegroup"); 
			}
            if ($fieldlist[$field]=='fl_regroupneg'){ 
				$valuetoshow=$langs->trans("FlRegroupNeg"); 
			}
// Fin 	Modif CCA 10/1/2018
// Modif CCA 14/2/2018 - ajout colone fl_type et fk_produit pour dictionnaire raison remise
            if ($fieldlist[$field]=='fl_type'){ 
				$valuetoshow=$langs->trans("DictFlType");
			}	
            if ($fieldlist[$field]=='fk_produit'){ 
				$valuetoshow=$langs->trans("DictFKProduit"); 
			}	
// Fin 	Modif CCA 14/2/2018  

// Modif CCA 10/12/2022 - ajout colonne  fk_cpt_bq 

            if ($fieldlist[$field]=='fk_cpt_bq '){ 
				$valuetoshow=$langs->trans("Fk_cpt_bq"); 
			}	
// Fin Modif CCA 10/12/2022 -
...
// Modif CCA 10/12/2022 - ajout colonne fl_regroup et fl_regroupneg , fk_cpt_bq dans c_paiement

			if ($value == 'fl_regroup' ) {
				$valuetoshow = $langs->trans('FlRegroup'); $cssprefix = 'center '; ;
			}
			if ($value == 'fl_regroupneg' ) {
				$valuetoshow = $langs->trans('FlRegroupNeg'); $cssprefix = 'center '; ;
			}
			if ($value == 'fk_cpt_bq' ) {
				$valuetoshow = $langs->trans('Fk_cpt_bq'); $cssprefix = 'center '; ;
			}
// Fin Modif CCA 10/12/2022 
			// Show field title
			if ($showfield) {
...
// Modif CCA 15/12/2016 - ajout type couleur
							} elseif ($fieldlist[$field] == 'color'   or $fieldlist[$field] == 'couleur'  )			{
								print '<td nowrap="nowrap" style="background-color:#'.$obj->color.'" >'.$obj->color;
								print '</td>';
								$showfield=0;							
// Fin Modif CCA 15/12/2016 -
// Modif CCA 14/2/2018 - ajout colone fl_regroup et fl_regroupneg 							
							} elseif ($fieldlist[$field]=='fl_regroup' || $fieldlist[$field]=='fl_regroupneg' ) {	
                                $valuetoshow = yn($valuetoshow);							
// Fin Modif CCA 14/2/2018
// Modif CCA 14/2/2018 - ajout colonne fl_type et fk_produit
							} else if ($fieldlist[$field]=='fl_type' ) {								
								if ( $valuetoshow ==  1) $valuetoshow = $langs->trans('RemiseFix');
								else  $valuetoshow = $langs->trans('RemPourc');
							} elseif ($fieldlist[$field]=='fk_produit' ) {
								if (!empty($valuetoshow) ) {		
									require_once DOL_DOCUMENT_ROOT.'/core/class/html.formother.class.php';
									$wprod = new Product ($db);							
									$ret = $wprod->fetch($valuetoshow)	;
									$valuetoshow = $wprod-> ref . ' - '. $wprod->label;
									unset ($wprod);
								}
								else 
									 $valuetoshow = '';	
// Fin Modif CCA 14/2/2018	
// Modif CCA 10/12/2022 - ajout colonne  fk_cpt_bq 
							} elseif ($value=='fk_cpt_bq' ) {
								require_once DOL_DOCUMENT_ROOT.'/compta/bank/class/account.class.php';
								$wb = new Account ($db);
								$wb->fetch($obj->fk_cpt_bq);
								$valuetoshow = $wb->ref. ' - '. $wb->label;								
// Fin Modif CCA 10/12/2022	
...
function fieldList($fieldlist, $obj = '', $tabname = '', $context = '')
{
	...
	foreach ($fieldlist as $field => $value) {
		...
		if (in_array($value, array('code', 'libelle', 'type')) && $tabname == MAIN_DB_PREFIX."c_actioncomm" && in_array($obj->type, array('system', 'systemauto'))) {
			...
			print '</td>';
// Modif CCA 10/1/2018 - ajout colone fl_regroup et fl_regroupneg 							
		} else if ($fieldlist[$field]=='fl_regroup' || $fieldlist[$field]=='fl_regroupneg') {									
 			print '<td>';
...
// Modif CCA 15/12/2016 - ajout type couleur
		elseif ($fieldlist[$field] == 'color' or $fieldlist[$field] == 'couleur' )
		{
			...
			if (($action == 'edit' and $context == 'edit') or ($action == 'view')) {
				//print select_color(! empty($obj->couleur)?$obj->couleur:'', 'couleur')
				require_once DOL_DOCUMENT_ROOT.'/core/class/html.formother.class.php';
				$formother = New FormOther ($db);			
				print $formother->selectColor(! empty($obj->color)?$obj->color:'', 'color', '', 1);
				unset ($formother);
			}
			print '</td>';
		}
// Fin Modif CCA 15/12/2016 
// Modif CCA 14/2/2018 - ajout colone fl_type et fk_produit pour dictionnaire raison remise
		elseif ($fieldlist[$field] == 'fl_type') {										
 			print '<td>';
			require_once DOL_DOCUMENT_ROOT.'/custom/cglavt/class/cglFctCommune.class.php';
			$wfcom = new CglFonctionCommune ($db);
			print $wfcom->select_typeremise((! empty($obj->{$fieldlist[$field]})?$obj->{$fieldlist[$field]}:''),$fieldlist[$field],1);
			unset ($wfcom);
			print '</td>';									
		}
		elseif ($fieldlist[$field] == 'fk_produit') {									
			...
			print $form->select_produits_list((! empty($obj->{$fieldlist[$field]})?$obj->{$fieldlist[$field]}:''), $fieldlist[$field], $filtertype,$limit,$price_level,$filterkey,$status,$finished,$outputmode,$socid,$showempty );
			//select_produits_list($selected='',$htmlname='productid',$filtertype='',$limit=20,$price_level=0,$filterkey='',$status=1,$finished=2,$outputmode=0,$socid=0)
			print '</td>';									
		}
//Fin  Modif CCA 14/2/2018
// Modif CCA 10/12/2022 - ajout colonne fk_cpt_bq 
		elseif ($fieldlist[$field] == 'fk_cpt_bq') {									
 			print '<td>';
			require_once DOL_DOCUMENT_ROOT.'/core/class/html.form.class.php';
			$wf = new Form ($db);
			print $wf->select_comptes($obj->fk_cpt_bq,$value,0);	
			unset ($wf);				
			print '</td>';	
		}
// fin  Modif CCA 10/12/2022 
