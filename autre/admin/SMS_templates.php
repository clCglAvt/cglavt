 ...
 *
 * Modif CCA - 27/05/2019 - Ajout du Sujet et fichier joint en cas de mail multi-lignes
 * Modif CCA 29/05/2019    - Mail avec lignes de détail lors de la mise à jour d'un modèle déjà existant
 *
 * CCA 15/1/2020 Adaptation pour les SMS avec un bouton 'recopîe depuis un modèle de mail'
 *
...
/**
 *	    \file       htdocs/admin/SMS_templates.php
 *		\ingroup    core
 *		\brief      Page to administer SMS templates Identifiant 199
 */
...
$langs->loadLangs(array("errors","admin","sms","languages"));
...
$hookmanager->initHooks(array('Smstemplates'));

// Name of SQL tables of dictionaries
$tabname=array();
$tabname[199]= MAIN_DB_PREFIX."c_sms_templates";

// Nom des champs en resultat de select pour affichage du dictionnaire
$tabfield=array();
$tabfield[199]= "label,lang,type_template,fk_user,private,position,content";
if (! empty($conf->global->MAIN_SMS_TEMPLATES_FOR_OBJECT_LINES)) $tabfield[199].=',content_lines';

// Nom des champs d'edition pour modification d'un enregistrement
$tabfieldvalue=array();
$tabfieldvalue[199]= "label,lang,type_template,fk_user,private,position,content";
if (! empty($conf->global->MAIN_SMS_TEMPLATES_FOR_OBJECT_LINES)) $tabfieldvalue[199].=',content_lines';

// Nom des champs dans la table pour insertion d'un enregistrement
$tabfieldinsert=array();
$tabfieldinsert[199]= "label,lang,type_template,fk_user,private,position, content";
if (! empty($conf->global->MAIN_SMS_TEMPLATES_FOR_OBJECT_LINES)) $tabfieldinsert[199].=',content_lines';
$tabfieldinsert[199].=',entity';     // Must be at end because not into other arrays
...
// Set MAIN_SMS_TEMPLATES_FOR_OBJECT_LINES to allow edit of template for lines
require_once DOL_DOCUMENT_ROOT.'/core/class/html.formsms.class.php';
$formsms=new FormSms($db);

//CCA========================Place de la doc des variable de substitution;

if (empty($conf->global->MAIN_SMS_TEMPLATES_FOR_OBJECT_LINES))
{
    $tmp=formsms::getAvailableSubstitKey('formesms');
...
{
    $tmp=formsms::getAvailableSubstitKey('formesmswithlines');
    $tmp['__(AnyTranslationKey)__']='Translation';
    $helpsubstit = $langs->trans("AvailableVariables").':<br>';
    $helpsubstitforlines = $langs->trans("AvailableVariables").':<br>';
    foreach($tmp as $key => $val)
    ...
    $tmp=formsms::getAvailableSubstitKey('formesmsforlines');
    foreach($tmp as $key => $val)
    {
    	$helpsubstitforlines.=$key.' -> '.$val.'<br>';
    }
...
$tabhelp=array();
$tabhelp[199] = array('content'=>$helpsubstit,'content_lines'=>$helpsubstitforlines,'type_template'=>$langs->trans("TemplateForElement"),'private'=>$langs->trans("TemplateIsVisibleByOwnerOnly"), 'position'=>$langs->trans("PositionIntoComboList"));

// List of check for fields (NOT USED YET)
$tabfieldcheck=array();
$tabfieldcheck[199] = array();
...
if (!empty(GETPOST('actioncopie'))) {
	// Copier l'id Mail en Nouel SMS et afficher le nouvel Sms en modification
	$CpModelId = GETPOST('CpModelId');
	
	$sql = 'INSERT INTO '.MAIN_DB_PREFIX.'c_sms_templates (label, type_template, lang, fk_user, private, position,  content_lines, content, enabled, active)';
	$sql .= ' SELECT  label, type_template, lang, fk_user, private, position,  content_lines, content, enabled, active';
	$sql.=" FROM ".MAIN_DB_PREFIX."c_email_templates";
	$sql.=" WHERE rowid = '".$CpModelId."'";
	
    dol_syslog("SMS_templates::actioncopie ");
    $resql=$db->query($sql);
    if ($resql) {
		$rowid = $db->last_insert_id(MAIN_DB_PREFIX."c_sms_templates");
		$action = 'edit';
	}
}
...
print '<br>CCA ========================== Voir routine sms_admin_prepare_head equivalent email_admin_prepare_head de admin.lib, elle devrati être appelée dans admin/sms.php';
print '<br>CCA ========================== elle préparre les onglets de la page de configuration des mails';
...
$sql="SELECT rowid as rowid, label, type_template, lang, fk_user, private, position,  content_lines, content, enabled, active";
...
if ($action <> 'edit') {
	...
	$reshook = $hookmanager->executeHooks('createSmsTemplateFieldlist', $parameters, $obj, $tmpaction); // Note that $action and $object may have been modified by some hooks
	...
	if (! empty($conf->global->MAIN_SMS_TEMPLATES_FOR_OBJECT_LINES)) { $fieldsforcontent = array('content','content_lines'); }
	...
	foreach ($fieldsforcontent as $tmpfieldlist)
	{
		...
		if ($context != 'hide') {
			...
			if (empty($conf->global->FCKEDITOR_ENABLE_MAIL))
				$okforextended = false;
			$doleditor = new DolEditor($tmpfieldlist, (! empty($obj->{$tmpfieldlist}) ? $obj->{$tmpfieldlist} : ''), '', 120, 'dolibarr_Smss', 'In', 0, false, $okforextended, ROWS_4, '90%');
...
if ($action != 'edit') {
	// Recopier un modèle mail en modèle Sms
	$wfc = new FormCglCommun ($db);
	print '<div>';
	print '<table>';
	print '<tr><td>';
	// Titre
	print $langs->trans("TiCopieMailenSms");
	print '</td><td>';
	// Select
	print $wfc->select_model('', 'tous', 'CpModelId', 1);
	print '</td><td align="center">';
	print '<input type="submit" class="button" name="actioncopie" value="' . $langs->trans("BtCopieMailenSms") . '">';
...
if ($resql)
{
    ...
    if (empty($conf->global->MAIN_SMS_TEMPLATES_FOR_OBJECT_LINES)) print '<td class="liste_titre"></td>';
    // Action column
    ...
    if ($num)
    {
        ...
        while ($i < $num)
        {
            ...
            if ($action == 'edit' && ($rowid == (! empty($obj->rowid)?$obj->rowid:$obj->code)))
            {
                ...
                $reshook=$hookmanager->executeHooks('editSmsTemplateFieldlist', $parameters, $obj, $tmpaction);    // Note that $action and $object may have been modified by some hooks
                ...
                foreach ($fieldsforcontent as $tmpfieldlist)
                {
                    ...
                    if ($showfield) {
						// Show line for  content et content_lines
                        ...
                        if ($tmpfieldlist == 'content')
                        {
                        	...
                        	$doleditor = new DolEditor($tmpfieldlist.'-'.$rowid, (! empty($obj->{$tmpfieldlist}) ? $obj->{$tmpfieldlist} : ''), '', 140, 'dolibarr_Smss', 'In', 0, false, $okforextended, ROWS_6, '90%');							
                        	print $doleditor->Create(1);
						}
						if ($tmpfieldlist == 'content_lines')
                        {
                        	print $form->textwithpicto($langs->trans("ContentLines"), $tabhelp[$id][$tmpfieldlist], 1, 'help', '', 0, 2, $tmpfieldlist) . '<br>';
                        	$okforextended = true;
                        	if (empty($conf->global->FCKEDITOR_ENABLE_MAIL)) $okforextended = false;
                        	$doleditor = new DolEditor($tmpfieldlist.'-'.$rowid, (! empty($obj->{$tmpfieldlist}) ? $obj->{$tmpfieldlist} : ''), '', 140, 'dolibarr_Smss', 'In', 0, false, $okforextended, ROWS_6, '90%');
                        	print $doleditor->Create(1);
            ...
            else
            {
				...
				{
					$i++;
					continue;		// Sms template not qualified
				}
                ...
                $reshook=$hookmanager->executeHooks('viewSmsTemplateFieldlist', $parameters, $obj, $tmpaction);    // Note that $action and $object may have been modified by some hooks
                ...
                foreach ($fieldsforcontent as $tmpfieldlist)
                {
                    ...
                    if ($showfield) {
                        ...
                        if (empty($conf->global->FCKEDITOR_ENABLE_MAIL))
                            $okforextended = false;
                        $doleditor = new DolEditor($tmpfieldlist.'-'.$i, (! empty($obj->{$tmpfieldlist}) ? $obj->{$tmpfieldlist} : ''), '', 140, 'dolibarr_Smss', 'In', 0, false, $okforextended, ROWS_6, '90%', 1);
                        print $doleditor->Create(1);
                        print '</td>';
                        print '<td></td><td></td><td></td>';
...
function fieldList($fieldlist, $obj = '', $tabname = '', $context = '')
{
	...
	foreach ($fieldlist as $field => $value)
	{
		...
		elseif ($context == 'add'  && in_array($fieldlist[$field], array('content', 'content_lines'))) continue;
		elseif ($context == 'edit' && in_array($fieldlist[$field], array( 'content', 'content_lines'))) continue;
		elseif ($context == 'hide' && in_array($fieldlist[$field], array( 'content', 'content_lines'))) continue;
		else
		{
			$size=''; $class=''; $classtd='';
