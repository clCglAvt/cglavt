 ...
 *
 * MODIF CCA 21-09-2016 Selection ville  et département
 * MODIF CCA 21-09-2016 Mettre ligne vide à Type société
 * Modif CCA 15-12-2018 Correction car On n'arrive pas à interroger un client sans la connaissance de son type 
 *
...
// Modif CCA 15-12-2018 Version 8.0.3 - Correction car On n'arrive pas à interroger un client sans la connaissance de son type 
if ($search_type_thirdparty  == -1) $search_type_thirdparty = '';
// Fin Modif CCA
...
// MODIF CCA 21-09-2016 Selection ville et département
	...
	print select_town($search_town,'search_town','',0,'maxwidth100');
// FIN MODIF CCA 
	print '</td>';
}
// State
...
//	print '<input class="flat searchstring maxwidth50imp" type="text" name="search_state" value="'.dol_escape_htmltag($search_state).'">';
	print select_state($search_state,'search_state','',0,'maxwidth100');
// FIN MODIF CCA 	
	print '</td>';
}
// Region
...
$db->close();
// MODIF CCA 21-09-2016 Selection ville et département
	function select_town($selected,$htmlname='search_town',$htmloption='',$maxlength=0,$morecss='minwidth300')
	{
        ...
        $sql = "SELECT distinct town";
        $sql.= " FROM ".MAIN_DB_PREFIX."societe";
        $sql.= " WHERE  entity IN (".getEntity('societe', 1).")";
        $sql.= " ORDER BY town ASC";

        dol_syslog("list.php::select_town", LOG_DEBUG);
        $resql=$db->query($sql);
        if ($resql)
        {
            //$out.= '<select id="select'.$htmlname.'" class="flat selecttown'.($morecss?' '.$morecss:'').'" name="'.$htmlname.'" '.$htmloption.'>';
            $out.= '<select id="'.$htmlname.'" class="flat" name="'.$htmlname.'" '.$htmloption.'>';
            $num = $db->num_rows($resql);
            $i = 0;
            if ($num)
            {
                ...
                while ($i < $num)
                {
					...
					$town = $obj->town;
					if ($town == $selected) $out .= '<option value="'.$town.'" selected="selected">';
					else $out .= '<option value="'.$town.'" >';
					$out .= $town;
	...
	function select_state($selected,$htmlname='search_state',$htmloption='',$maxlength=0,$morecss='minwidth300')
	{
	     global $conf,$langs, $db;
        ...
        $sql = "SELECT distinct fk_departement, d.nom, code_departement ";
        $sql.= " FROM ".MAIN_DB_PREFIX."societe as s , " .MAIN_DB_PREFIX."c_departements as d";
        $sql.= " WHERE fk_departement = d.rowid AND  s.entity IN (".getEntity('societe', 1).")";
        $sql.= " ORDER BY d.nom  ASC";

        dol_syslog("list.php::select_state", LOG_DEBUG);
        $resql=$db->query($sql);
        if ($resql)
        {
            //$out.= '<select id="select'.$htmlname.'" class="flat selecttown'.($morecss?' '.$morecss:'').'" name="'.$htmlname.'" '.$htmloption.'>';
            ...
            if ($num)
            {
                ...
                while ($i < $num)
                {
					...
					if ($state_name  == $selected) $out .= '<option value="'.$obj->fk_departement .'" selected="selected">';
					else $out .= '<option value="'.$obj->fk_departement .'" >';
					$out .= $obj->nom.' ('.$obj->code_departement.') ' ;
