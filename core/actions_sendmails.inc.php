...
*
* Modif CCA 12/1/2020 - Utilisation par BU/Location - Stripe
*
...
/**
 *	\file			htdocs/custom/cglavt/core/actions_sendmails.inc.php
...
if (($action == 'send' || $action == 'relance') && ! $_POST['addfile'] && ! $_POST['removAll'] && ! $_POST['removedfile'] && ! $_POST['cancel'] && !$_POST['modelselected'])
{
	...
	if ($result > 0)
	{
		...
		$tmparray=array();
//if (!empty($tmparray)) print '<br>CCA -actions_sendamils---creation tableau --- sendtocc renseigné';
		if (trim($_POST['sendtocc']))
		{
			$tmparray[] = trim($_POST['sendtocc']);
		}
//if (!empty($tmparray)) print '<br>CCA - -actions_sendamils---_POST sendtocc --- sendtocc renseigné';
		if (count($receivercc) > 0)
		{
			foreach($receivercc as $key=>$val)
			{
				...
				{
					$tmparray[] = dol_string_nospecial($thirdparty->name, ' ', array(",")).' <'.$thirdparty->email.'>';
//if (!empty($tmparray)) print '<br>CCA - -actions_sendamils---thirdparty--- sendtocc renseigné';
				}
				// Recipient was provided from combo list
				elseif ($val == 'contact') // Id of contact
				{
					$tmparray[] = dol_string_nospecial($contact->name, ' ', array(",")).' <'.$contact->email.'>';
//if (!empty($tmparray)) print '<br>CCA - -actions_sendamils---contact--- sendtocc renseigné';
				}
				elseif ($val)	// Id du contact
				{
					$tmparray[] = $thirdparty->contact_get_property((int) $val, 'email');
//if (!empty($tmparray)) print '<br>CCA - -actions_sendamils---Id du contact--- sendtocc renseigné';
		...
		if (!empty($conf->global->MAIN_MAIL_ENABLED_USER_DEST_SELECT)) {
			...
			if (is_array($receiverccuser) && count($receiverccuser)>0)
			{
				...
				foreach($receiverccuser as $key=>$val)
				{
					$tmparray[] = $fuserdest->user_get_property($val, 'email');
					$sendtoccuserid[] = $val;
//if (!empty($tmparray)) print '<br>CCA - -actions_sendamils---foreach key:'.$key.'=> val:'.$val.'--- sendtocc renseigné';
				}
			}
		}
		...
		if (dol_strlen($sendto))
		{
           // Define $urlwithroot
			...
			$sendtobcc= GETPOST('sendtoccc','alpha');
...
/* dol_syslog( '<br> CCA ---------------------------------------------------------Destinataire: '.$sendto);

print '<br> CCA ----------------------------------------------------------MAIL constitution et construct CMailFile ';
print '<br> CCA ----------------------------------------------------------MAIL constitution -- subject:'.$subject;
print '<br> CCA ----------------------------------------------------------MAIL constitution -- sendto:'.$sendto;
print '<br> CCA ----------------------------------------------------------MAIL constitution -- from:'.$from;
print '<br> CCA ----------------------------------------------------------MAIL constitution -- filepath:'.$filepath;
print '<br> CCA ----------------------------------------------------------MAIL constitution -- mimetype:'.$mimetype;
print '<br> CCA ----------------------------------------------------------MAIL constitution -- filename:'.$filename;
print '<br> CCA ----------------------------------------------------------MAIL constitution -- sendtocc:'.$sendtocc;
print '<br> CCA ----------------------------------------------------------MAIL constitution -- sendtobcc:'.$sendtobcc;
print '<br> CCA ----------------------------------------------------------MAIL constitution -- deliveryreceipt:'.$deliveryreceipt;
print '<br> CCA ----------------------------------------------------------MAIL constitution -- trackid:'.$trackid;
print '<br> CCA ----------------------------------------------------------MAIL constitution -- sendcontext:'.$sendcontext;
print  '<br> CCA ----------------------------------------------------------MAIL constitution -- lg message:'.strlen($message);

dol_syslog( '<br>CCA -  Send Mail - client : '.$bull->TiersMail);
*/
			...
			if ($mailfile->error)
			{
//print  '<br> CCA ----------------------------------------------------------MAIL constitution -- Erreur sur message :'.$subject;
